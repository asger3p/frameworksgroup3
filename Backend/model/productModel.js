...
createProduct(body) {
    const { product_id, name, subheading, description, image, cuisine, type, sizes } = body;

    //validation that it recieves the correct parameters 
    if (!product_id || !name || !description || !image || !cuisine || !type || !Array.isArray(sizes)) {
      throw new Error("Missing required fields or invalid format");
    }
    //validating variable-types for sizes
    const validSizes = sizes.every((s) => typeof s.size === "string" && typeof s.price === "number");
    if (!validSizes) {
      throw new Error("Each size must be an object with a string size and numeric price");
    }
  ...
  updateProduct(id, body) {
    ...
    const productIndex = db.products.findIndex(product => product.product_id === id);
